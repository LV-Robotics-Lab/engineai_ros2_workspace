...
class ObservationManager {
 public:
  ObservationManager(const data::RlDanceParam& param);
  void SetProfile(const std::string& profile_key, const data::RlDanceMotionStateProfile& profile);
  void UpdateObservation(const Eigen::VectorXd& q_actual, const Eigen::VectorXd& qd_actual,
                         ...
                         const Eigen::VectorXd& default_joint_q, bool is_stable = false,
                         const std::optional<Eigen::Vector2d>& yaw_forward_vec = std::nullopt,
                         const std::optional<Eigen::Quaterniond>& ref_quat = std::nullopt,
                         const std::optional<Eigen::Quaterniond>& current_quat = std::nullopt);
 ...
 private:
  ...
  double CalculateDynamicCycleTime(const Eigen::Vector3d& command) const;
  ...
  // Get future frames reference (current + future 9 frames = 10 frames total)
  Eigen::VectorXd GetFutureFramesReference(int num_frames = 10) const;
  // Helper functions for GetCurrentObservation
  Eigen::VectorXd BuildProprioceptiveScale(int proprio_dim, int time_steps) const;
  void ApplyGoalScaling(Eigen::VectorXd& observation, int obs_rows, int quat_error_total, int goal_rows, int goal_cols) const;

  const data::RlDanceParam& param_;
  std::string profile_key_;
  std::string observation_type_;
  const data::RlDanceMotionStateProfile* current_profile_ = nullptr;
  const data::RlDanceObservationConfig* current_obs_cfg_ = nullptr;
  ...
  Eigen::MatrixXd quat_error_history_;   // 四元数差值历史 (当use_quat_error启用时)
  // Eigen::MatrixXd yaw_forward_history_;  // Yaw方向向量历史

  // 历史buffer管理
  std::vector<std::pair<Eigen::MatrixXd*, int>> history_buffers_;  // {buffer指针, 维度}

  std::map<std::string, Eigen::MatrixXd> goal_buffers_;   // 目标buffer（mimic/rl_locomotion/rl_locomotion_saw）
  ...
  // Gait phase for locomotion (thread-safe member variable)
  double gait_phase_ = 0.0;
  std::string last_profile_key_;  // 用于检测profile切换
  double runner_period_ = 0.02;   // runner周期时间，默认值
  
  // Initial quaternion offset for quat_error correction
  // Stores the quaternion difference between robot initial pose and reference initial pose
  std::optional<Eigen::Quaterniond> initial_quat_offset_;  // q_offset = q_robot_init^{-1} * q_ref_init
  bool initial_quat_offset_computed_ = false;  // Flag to indicate if offset has been computed
};
