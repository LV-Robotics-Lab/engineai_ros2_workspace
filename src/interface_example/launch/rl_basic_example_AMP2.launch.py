...
def generate_launch_description():
    ...
    amp2_config_file = os.path.join(config_dir, 'rl_basic_param_AMP2.yaml')
    if not os.path.exists(amp2_config_file):
        raise FileNotFoundError(f"AMP2 config file not found: {amp2_config_file}")
    print(f"Using AMP2 config file: {amp2_config_file}")
    ...
    hardware_node = Node(
        ...
        executable='rl_basic_example_AMP2',
        name='rl_basic_example_AMP2',
        arguments=[config_dir, 'rl_basic_param_AMP2.yaml'],
        output='screen',
        emulate_tty=True,
    )
